
.. _file_pcl_geometry_include_pcl_geometry_polygon_operations.h:

File polygon_operations.h
=========================

|exhale_lsh| :ref:`Parent directory <dir_pcl_geometry_include_pcl_geometry>` (``pcl\geometry\include\pcl\geometry``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS

.. contents:: Contents
   :local:
   :backlinks: none

Definition (``pcl\geometry\include\pcl\geometry\polygon_operations.h``)
-----------------------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_pcl_geometry_include_pcl_geometry_polygon_operations.h.rst





Includes
--------


- ``impl/polygon_operations.hpp``

- ``pcl/point_cloud.h``

- ``planar_polygon.h`` (:ref:`file_pcl_geometry_include_pcl_geometry_planar_polygon.h`)



Included By
-----------


- :ref:`file_pcl_geometry_include_pcl_geometry_impl_polygon_operations.hpp`




Namespaces
----------


- :ref:`namespace_pcl`


Functions
---------


- :ref:`exhale_function_namespacepcl_1a838f3e1252e58be919249a1bddff97e6`

- :ref:`exhale_function_namespacepcl_1a10c67ad60b6d7bec3243f3ab850ecb7d`

