
.. _file_pcl_surface_include_pcl_surface_processing.h:

File processing.h
=================

|exhale_lsh| :ref:`Parent directory <dir_pcl_surface_include_pcl_surface>` (``pcl\surface\include\pcl\surface``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS

.. contents:: Contents
   :local:
   :backlinks: none

Definition (``pcl\surface\include\pcl\surface\processing.h``)
-------------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_pcl_surface_include_pcl_surface_processing.h.rst





Includes
--------


- ``pcl/PolygonMesh.h``

- ``pcl/pcl_base.h``

- ``pcl/point_cloud.h``

- ``pcl/surface/impl/processing.hpp``



Included By
-----------


- :ref:`file_pcl_surface_include_pcl_surface_bilateral_upsampling.h`

- :ref:`file_pcl_surface_include_pcl_surface_ear_clipping.h`

- :ref:`file_pcl_surface_include_pcl_surface_mls.h`

- :ref:`file_pcl_surface_include_pcl_surface_vtk_smoothing_vtk_mesh_quadric_decimation.h`

- :ref:`file_pcl_surface_include_pcl_surface_vtk_smoothing_vtk_mesh_smoothing_laplacian.h`

- :ref:`file_pcl_surface_include_pcl_surface_vtk_smoothing_vtk_mesh_smoothing_windowed_sinc.h`

- :ref:`file_pcl_surface_include_pcl_surface_vtk_smoothing_vtk_mesh_subdivision.h`




Namespaces
----------


- :ref:`namespace_pcl`


Classes
-------


- :ref:`exhale_class_classpcl_1_1_cloud_surface_processing`

- :ref:`exhale_class_classpcl_1_1_mesh_processing`

