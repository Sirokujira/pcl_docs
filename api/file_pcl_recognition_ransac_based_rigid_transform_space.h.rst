
.. _file_pcl_recognition_ransac_based_rigid_transform_space.h:

File rigid_transform_space.h
============================

|exhale_lsh| :ref:`Parent directory <dir_pcl_recognition_ransac_based>` (``pcl\recognition\ransac_based``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS

.. contents:: Contents
   :local:
   :backlinks: none

Definition (``pcl\recognition\ransac_based\rigid_transform_space.h``)
---------------------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_pcl_recognition_ransac_based_rigid_transform_space.h.rst





Includes
--------


- ``list`` (:ref:`file_pcl_io_openni2_openni2_frame_listener.h`)

- ``map`` (:ref:`file_pcl_visualization_common_actor_map.h`)

- ``model_library.h`` (:ref:`file_pcl_recognition_model_library.h`)

- ``pcl/pcl_exports.h``

- ``simple_octree.h`` (:ref:`file_pcl_recognition_ransac_based_simple_octree.h`)



Included By
-----------


- :ref:`file_pcl_recognition_ransac_based_obj_rec_ransac.h`

- :ref:`file_pcl_recognition_rigid_transform_space.h`




Namespaces
----------


- :ref:`namespace_pcl`

- :ref:`namespace_pcl__recognition`


Classes
-------


- :ref:`exhale_class_classpcl_1_1recognition_1_1_rigid_transform_space`

- :ref:`exhale_class_classpcl_1_1recognition_1_1_rotation_space`

- :ref:`exhale_class_classpcl_1_1recognition_1_1_rotation_space_cell`

- :ref:`exhale_class_classpcl_1_1recognition_1_1_rotation_space_cell_1_1_entry`

- :ref:`exhale_class_classpcl_1_1recognition_1_1_rotation_space_cell_creator`

- :ref:`exhale_class_classpcl_1_1recognition_1_1_rotation_space_creator`


Typedefs
--------


- :ref:`exhale_typedef_ransac__based_2rigid__transform__space_8h_1a2f9188b0e5df273b644639fbdc7c7509`

- :ref:`exhale_typedef_ransac__based_2rigid__transform__space_8h_1a54295ada2a796436cb2ecf587e0b9a8a`

