...
def run_doxygen(folder):
    ...
    try:
        ...
        if platform.system() == "Windows":
            ...
            # retcode = subprocess.call("cd %s/pcl/build; %s/cmake --build . -- doc tutorials advanced; cd -" % (folder, cmake.CMAKE_BIN_DIR), stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, shell=True)
            if retcode < 0:
...
if read_the_docs_build:
    ...
    read_the_docs_version_name = os.environ.get('READTHEDOCS_VERSION', 'latest')
    #rootpath = os.path.join(os.getcwd(), '../conda', read_the_docs_version_name)
    conda_rootpath = os.environ.get('CONDA_ENVS_PATH')
    rootpath = os.path.join(conda_rootpath, read_the_docs_version_name)
