
.. _file_E__pcl_docs_pcl_gpu_features_src_utils_vector_operations.hpp:

File vector_operations.hpp
==========================

|exhale_lsh| :ref:`Parent directory <dir_E__pcl_docs_pcl_gpu_features_src_utils>` (``E:\pcl_docs\pcl\gpu\features\src\utils``)

.. |exhale_lsh| unicode:: U+021B0 .. UPWARDS ARROW WITH TIP LEFTWARDS

.. contents:: Contents
   :local:
   :backlinks: none

Definition (``E:\pcl_docs\pcl\gpu\features\src\utils\vector_operations.hpp``)
-----------------------------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_E__pcl_docs_pcl_gpu_features_src_utils_vector_operations.hpp.rst





Includes
--------


- ``pcl/gpu/features/device/rodrigues.hpp``

- ``pcl/gpu/utils/device/vector_math.hpp``






Namespaces
----------


- :ref:`namespace_pcl`

- :ref:`namespace_pcl__device`


Functions
---------


- :ref:`exhale_function_vector__operations_8hpp_1a88588f028f0250a361e9bc8a08129ae2`

- :ref:`exhale_function_vector__operations_8hpp_1a47482a10260868e88ac83ac099f3e75f`

- :ref:`exhale_function_vector__operations_8hpp_1a31b3925661a44b88b42216b4f124a983`

- :ref:`exhale_function_vector__operations_8hpp_1aeea1ab006c5e7969d99e4e98b9130000`

- :ref:`exhale_function_vector__operations_8hpp_1a099f9e97424f5830b0b2a8a19fbc1250`

